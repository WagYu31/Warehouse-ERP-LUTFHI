...
func (h *Handler) GetSuppliers(c *gin.Context) {
	...
	args := []interface{}{}
	if search != "" {
		q += ` AND (name ILIKE $1 OR email ILIKE $1)`
...
func (h *Handler) CreateSupplier(c *gin.Context) {
	...
	id := uuid.New().String()
	h.DB.Exec(`INSERT INTO suppliers (id,code,name,email,phone,city) VALUES ($1,$2,$3,$4,$5,$6)`,
...
func (h *Handler) UpdateSupplier(c *gin.Context) {
	...
	c.ShouldBindJSON(&b)
	h.DB.Exec(`UPDATE suppliers SET name=$1,email=$2,phone=$3,city=$4,updated_at=$5 WHERE id=$6`,
...
func (h *Handler) DeleteSupplier(c *gin.Context) {
	h.DB.Exec(`UPDATE suppliers SET is_active=false WHERE id=$1`, c.Param("id"))
...
func (h *Handler) GetItems(c *gin.Context) {
	...
	if warehouseID != "" {
		rows, err = h.DB.Query(`
			...
			LEFT JOIN item_stocks s ON i.id = s.item_id AND s.warehouse_id = $1
			WHERE i.is_active = true AND (i.name ILIKE $2 OR i.sku ILIKE $2)
		...
		rows, err = h.DB.Query(`
			...
			WHERE i.is_active = true AND (i.name ILIKE $1 OR i.sku ILIKE $1)
			GROUP BY i.id, i.sku, i.name, i.min_stock, i.price, i.is_active, c.name, u.abbreviation
...
func (h *Handler) GetItem(c *gin.Context) {
	...
	err := h.DB.QueryRow(`SELECT id,sku,name,min_stock,price,is_active FROM items WHERE id=$1`, id).
...
func (h *Handler) CreateItem(c *gin.Context) {
	...
	id := uuid.New().String()
	_, err := h.DB.Exec(`INSERT INTO items (id,sku,name,category_id,unit_id,min_stock,price) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
...
func (h *Handler) UpdateItem(c *gin.Context) {
	...
	c.ShouldBindJSON(&b)
	h.DB.Exec(`UPDATE items SET name=$1,min_stock=$2,price=$3,updated_at=$4 WHERE id=$5`,
